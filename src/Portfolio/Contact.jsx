...
const EMAILJS_TEMPLATE_ID = "template_e68m118";   // EmailJS dashboard > Email Templates
...
export default function Contact() {
  ...
  const handleSubmit = async () => {
    ...
    if (!name.trim() || !email.trim() || !message.trim()) {
      setStatus("error");
      setErrorMsg("Sabhi fields fill karo — koi bhi blank nahi hona chahiye.");
      setTimeout(() => setStatus("idle"), 3500);
      return;
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      setStatus("error");
      setErrorMsg("Valid email address enter karo.");
      setTimeout(() => setStatus("idle"), 3500);
      return;
    }
